...
export function EarlyAccess() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      setSubmitted(true);
      toast.success('You\'re in! Check your email for early access updates.');
      setEmail('');
    }, 1000);
  };

  return (
    <section id="early-access" className="py-12 sm:py-20 px-4 bg-gradient-to-br from-black via-gray-900 to-black text-white relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-yellow-400/20 border border-yellow-400/30 rounded-full text-sm font-medium text-yellow-300 mb-4">
            <Sparkles className="w-4 h-4" />
            <span>Limited Early Access</span>
          </div>

          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold leading-tight px-4">
            Be the First to Flex Your Stats
          </h2>

          <p className="text-lg sm:text-xl text-gray-300 max-w-2xl mx-auto px-4">
            Join 500+ drivers already on the waitlist. Get early access when we launch, 
            exclusive badges, and your name on the leaderboard from day one.
          </p>
